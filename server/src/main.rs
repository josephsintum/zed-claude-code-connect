...
fn main() -> Result<()> {
    ...
    // The LSP loop owns a plain OS thread that is not joined here, and the
    // blocking pool is empty because nothing on this side of the process uses it
    // any more. The bound stays as a backstop: when we stop for a reason other
    // than stdin closing -- a signal, Zed dying -- that read is still parked, and
    // an unbounded wait would never return. The lock file is already gone by this
    // point; nothing of value is lost by giving up on a parked read.
...
async fn serve(config: Config) -> Result<()> {
    info!(
        "claude-code-connect {} starting for {}",
    ...
    let reason = tokio::select! {
        ...
        // The handles rather than their locks: a lock guard is not `Send` and
        // the loop runs on a thread of its own. Both are wrapped so the LSP
        // thread reads and writes whole frames, not bytes.
        _ = handle.run_lsp(
            std::io::BufReader::with_capacity(64 * 1024, std::io::stdin()),
            std::io::BufWriter::with_capacity(64 * 1024, std::io::stdout()),
        ) => "Zed closed the LSP channel",

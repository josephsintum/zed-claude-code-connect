...
```
keypress ──> Zed task ──> zed-claude-ide-server at-mention
...
cp target/release/zed-claude-ide-server ~/.local/bin/
```
...
zed-claude-ide-server at-mention --worktree "$PWD"
```
